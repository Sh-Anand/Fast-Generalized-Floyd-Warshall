...
for impl in range(2):
    for j in range(semi_rings):
        ...
        with open("intermediate_perf.csv", "a") as res_dump_file:
            res_dump_file.write(str(fw[j]) + " " + str(implementations[impl]) + "\n")
        ...
        while n <= MAX_N:
            ...
            for i in range(repetitions_for_confidence):
                    ...
                    output = run("%s %d %d %d" % (executable_abs_path, n, j, impl), capture_output=True, shell=True).stdout.decode("utf-8")
